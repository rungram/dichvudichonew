...
class CKFinder_Connector_CommandHandler_CreateZip extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
	...
	public function buildXml()
	{
		...
		if (file_exists($zipFilePath) && isset($_POST['download']) && $_POST['download'] == 'true') {
			...
			if (file_exists($zipFilePath) && (! isset($_POST['fileExistsAction']) || ! in_array($_POST['fileExistsAction'], array(
				'autorename',
				'overwrite'
			)))) {
...
class CKFinder_Connector_CommandHandler_DownloadZip extends CKFinder_Connector_CommandHandler_CreateZip
{
	...
	protected function sendZipFile()
	{
		...
		if (strpos($user_agent, "MSIE") !== false) {
			$encodedName = str_replace(array(
				"+",
				"%2E"
			), array(
				" ",
				"."
			), urlencode($encodedName));
		}
...
if (extension_loaded('zip')) {
	...
	$config['Hooks']['BeforeExecuteCommand'][] = array(
		$CommandHandler_UnzipHere,
		"onBeforeExecuteCommand"
	);
	$config['Hooks']['BeforeExecuteCommand'][] = array(
		$CommandHandler_UnzipTo,
		"onBeforeExecuteCommand"
	);
	$config['Hooks']['BeforeExecuteCommand'][] = array(
		$CommandHandler_CreateZip,
		"onBeforeExecuteCommand"
	);
	$config['Hooks']['BeforeExecuteCommand'][] = array(
		$CommandHandler_DownloadZip,
		"onBeforeExecuteCommand"
	);
	$config['Plugins'][] = 'zip';
}
